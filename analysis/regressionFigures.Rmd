...
# %>% 
#     mutate(.category = case_when(
#     as.numeric(.category) == 1 ~ "Strongly Oppose",
#     as.numeric(.category) == 2 ~ "Oppose",
#     as.numeric(.category) == 3 ~ "Support",
#     as.numeric(.category) == 4 ~ "Strongly Support",
#     TRUE ~ NA
#   ) ) %>%
  #%>%# order .category
...
And the wrapper for the marginal effects **ggMarginal**
...
process_posterior <- function(model_data, recoder = "support5") {
  model_data %>%
    ...
    mutate(.category = case_when(
      ...
      recoder == "serious4" &  as.numeric(.category) == 5 ~ "Strongly Support",
      recoder == "serious4" &  as.numeric(.category) == 4 ~ "Somewhat Support",
      recoder == "serious4" &  as.numeric(.category) == 3 ~ "Neither Support nor Oppose",
      recoder == "serious4" &  as.numeric(.category) == 2 ~ "Somewhat Oppose",
      recoder == "serious4" &  as.numeric(.category) == 1 ~ "Strongly Oppose",
      TRUE ~ NA
    )) 
}

a1 <- posterior_pme(model = models[[1]]) %>% process_posterior(recoder = "support5")
a2 <- posterior_pme(model = models[[2]]) %>% process_posterior()
a3 <- posterior_pme(model = models[[3]]) %>% process_posterior()
```
